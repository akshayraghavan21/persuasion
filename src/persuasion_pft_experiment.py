...
def parse_args() -> argparse.Namespace:
    ...
    ## Project Params
    parser.add_argument("--wandb_project", type=str, default='experiment_gaia_llama2-7b_pft')
    parser.add_argument("--model_name", type=str, default="/gaia_data/pretrained_models/llama2-7b-hf/")
    parser.add_argument("--data_file", type=str, default="pft_all_pos_neg_comment_v001.json")
    parser.add_argument("--project_data_dir", type=str, default="../data/")
    parser.add_argument("--project_output_dir", type=str, default="../output_it")
    ...
    ## Dataset Prep
    parser.add_argument("--preprocess_datasets", type=bool, default=False)
    parser.add_argument("--preprocess_datasets_fn", type=str, default="template_edit_dataset")
    parser.add_argument("--template_prefix", type=str, default="When given a piece of text, your task is to craft a persuasive response that encourages the reader to reconsider or adjust their position. Employ strategies that include (but are not limited to) presenting well-supported arguments, appealing to both emotions and logic, and thoughtfully addressing counterarguments. Adapt your tone to the context, ensuring your response is assertive yet respectful. Your goal is to move the reader's stance, ideally toward your perspective, but complete reversal isn't always necessary — nudging their viewpoint or sparking doubt can be just as valuable.")
...
def get_training_args(args: argparse.Namespace, output_dir: str) -> SFTConfig:
    """
...
def train_pre_fine_tune(args: argparse.Namespace) -> float:
    """
    ...
    try:
        with wandb.init(project=args.wandb_project, config=vars(args), dir=args.project_output_dir) as run:
            ...
            tokenizer_output_dir = os.path.join(run_output_dir, "final_tokenizer")
            trainer_output_dir = os.path.join(run_output_dir, "final_trainer")
            
            trainer.model.save_pretrained(model_output_dir)
            tokenizer.save_pretrained(tokenizer_output_dir)
            trainer.save_pretrained(trainer_output_dir)
...
if __name__ == "__main__":
    ...
    ## Train DPO
    final_eval_loss = train_pre_fine_tune(args)
