...
wandb_run_ = wandb.init(project='experiment_gaia_llama2-7b_tests', config={
    "learning_rate":5e-5,
...
wandb_run_.log({"sample_level_metrics_test": "hello"})

model_name = "/gaia_data/pretrained_models/llama2-7b-hf/"
# import pdb; pdb.set_trace()
output_dir_suffix = wandb_run_.dir.split('/')[-2].replace("run-", "").replace("-", "_")
run_output_dir = f"../output/run_{output_dir_suffix}"
model_op_dir = os.path.join(run_output_dir, f"final_model_{output_dir_suffix}")
loss_history_file = os.path.join(run_output_dir, f"loss_history_{output_dir_suffix}.csv")
...
# eval_sample_gen_txt_table_data = wandb.Table(columns=['eval_sample_index', 'epoch', 'prompt', 'generated_text', 'chosen', 'rejected'])
...
timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
persuasion_dpo_trainer.model.save_pretrained(model_op_dir)

wandb_run_.log({"sample_level_metrics_table_data": persuasion_dpo_trainer.sample_level_metrics_table_data})
print("Logged Sample Level Metrics Tabular Data to Wandb")

wandb_run_.log({"eval_sample_gen_txt_table_data": persuasion_dpo_trainer.eval_sample_gen_txt_table_data})
...
run_loss_history.to_csv(loss_history_file)
