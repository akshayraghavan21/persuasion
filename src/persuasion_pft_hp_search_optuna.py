import optuna
from persuasion_it_experiment import train_pre_fine_tune, parse_args
import wandb
import torch
import gc
import json
...
def objective(trial):
    ...
    args.num_train_epochs = trial.suggest_int('num_train_epochs', 2, 5) 
    args.lora_r = trial.suggest_categorical('lora_r', [8, 16])
    
    # Run the training
    try:
        # Run the training
        best_metric, wandb_run_id = train_instruction_tune(args)
...
if __name__ == "__main__":
    optimize_hyperparameters(n_trials=2)